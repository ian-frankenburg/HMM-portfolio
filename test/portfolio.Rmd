...
sampler = function(y, nugget, m){
  r <- NULL
  attempt <- 1
  while( is.null(r) && attempt <= 3 ) {
    attempt <- attempt + 1
    try(
       r <- gibbs(niter = 10000, burnin = 1000,
                      y = y, Sigma0 = Sigma00, v0 = v00, mu0 = mu00, S0 = S00, h=1, nugget, m)
    )
  }
  return(r)
}
...
m = c(0,.01,.025,.05,.1)
ptm <- Sys.time(); fit = sampler(y,1e-3,m=m); print(Sys.time() - ptm)

# axisLims = apply(fit$pf_weights, 3, density)
for(i in 1:p){
  # hist(fit$pf_weights[i,,1],probability = T, main=symbols[i], xlab="Weight", ylim=c(0,3), col=1,xlim=c(-5,5))
  plot(density(fit$pf_weights[i,,1]), col = 1,type="l", lwd=1)
  for(j in 2:length(m)){
    # hist(fit$pf_weights[i,,j], probability = T, add=T, col=j)
    lines(density(fit$pf_weights[i,,j]), col = j,type="l", lwd=1)
  }
  legend(2,2,legend = m,fill = 1:length(m),title = "Returns", cex=.75)
}
...
matplot(apply(fit$y_pred, 1:2, median),type="l")
matplot(y[(nrow(y)-9):nrow(y),], type="l")
...
$$
\begin{align}
\max_{\omega} & & \omega^\top\mu_{T+1}\\
\text{subject to} &  &\omega^\top\Sigma_{T+1}\omega\leq\sigma^{2\star}\\
\text{and} & & \omega^\top1  = 1
\end{align}
$$
Bayesian simulation-based portfolio resampling I can form a Monte Carlo sample from 
